...
public class Ex00_Oop {
	...
	public static void main(String[] args) {
		...
		//클래스에서는 인스턴트(instance)라고 부른다.
		
		//객체 특성 3가지
		//캡슐화(Encapsulation) : 정보은닉
		//상송(Inheritance) : 부모 - 자식
		//다형성(Polymorphism) : 상속에서 사용, 같은 코드 - 다른 출력
		...
		(1)필드 (field) : 속성, 상태, 특성 나타낼 때 필드 사용 - 명사형
							필드 대신 멤버변수라고도 함
							값의 상태를 관리
							데이터를 저장하는 공간
							int size = 6;
							
		(2)메소드(method) : 동작, 행동, 기능 - 동사형
									특정 기능을 정의한 블록
									메소드가 호출되면 자신이 가지고 있는 기능을 수행
									기능 수행이 끝나면 리턴값 존재
									
		(3)생성자(constructor) : 객체를 만들 때 따라야 하는 규칙 정의
										객체 생성을 도와주는 블록
										생성자 이름은 클래스 이름과 동일
